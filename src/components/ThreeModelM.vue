<template>
    <div ref="container">
      <ElectricMeterCard
        ...
        :key="model.userData.meterId"
        :meterId="model.userData.meterId"
        :deviceName="model.userData.deviceName"
        :currents="model.userData.currents"
        :power="model.userData.power"
  ...
  */
  ...
  const animate = () => {
    requestAnimationFrame(animate);
    models.forEach(model => updatePosition(model, camera, renderer));  // Update position for all models
    // adjustForOverlaps(); // Adjust positions to avoid overlaps
    controls.update();
    renderer.render(scene, camera);
  };
...
function updatePosition(model, camera, renderer) {
  ...
  // Update the styling directly in the model's userData for reactivity
  // cardPositions[model.userData.meterId] = { x, y, width: 200, height: 100 }; // Assuming fixed size for cards
  // model.userData.style = {
  //   position: 'absolute',
  //   transform: `translate(-50%, -50%) translate(${x}px,${y}px)`,
  //   display: 'block'
  ...
  // Save the positions and dimensions for overlap checks
  // cardPositions[model.userData.meterId] = { x, y, width:100, height:80 };
}

// function adjustForOverlaps() {
//   const keys = Object.keys(cardPositions);
//   keys.forEach((key, index) => {
//     let pos = cardPositions[key];
//     for (let i = 0; i < index; i++) {
//       let otherKey = keys[i];
//       let otherPos = cardPositions[otherKey];
//       if (isOverlapping(pos, otherPos)) {
//         pos.y += otherPos.height; // Move down by the height of the overlapping card
//         // Update the style to reflect the new position
//         const model = models.find(m => m.userData.meterId === key);
//         if (model) {
//           model.userData.style.transform = `translate(-50%, -50%) translate(${pos.x}px, ${pos.y}px)`;
//         }
//       }
//     }
//   });
...
//         let keyI = keys[i];
//         let keyJ = keys[j];
//         let posI = cardPositions[keyI];
//         let posJ = cardPositions[keyJ];
//         if (isOverlapping(posI, posJ)) {
//           // Calculate new Y position to move the overlapping card below
//           posJ.y = posI.y + posI.height + 10; // Adding 10 pixels for some spacing
//           updateCardPosition(keyJ, posJ.x, posJ.y);
//           adjustmentsMade = true;
//         }
//       }
//     }
//   } while (adjustmentsMade); // Keep adjusting until no overlaps are detected
// }


// function updateCardPosition(key, x, y) {
//   const model = models.find(m => m.userData.meterId === key);
//   if (model) {
//     model.userData.style.transform = `translate(-50%, -50%) translate(${x}px, ${y}px)`;
//   }
// }
